...
/*   Updated: 2022/03/11 02:01:32 by dbanfi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_screen ft_screen_maker(t_camera camera)
{
	...
	printf("tmp0 %f %f %f\n", fixed2double(temp[0].n_vec.x), fixed2double(temp[0].n_vec.y), fixed2double(temp[0].n_vec.z));
	printf("tmp1 %f %f %f\n", fixed2double(temp[1].n_vec.x), fixed2double(temp[1].n_vec.y), fixed2double(temp[1].n_vec.z));
	printf("tmp2 %f %f %f\n", fixed2double(temp[2].n_vec.x), fixed2double(temp[2].n_vec.y), fixed2double(temp[2].n_vec.z));
	printf("siye %f %f %f\n", fixed2double(temp[0].size), fixed2double(temp[1].size), fixed2double(temp[2].size));
	...
	printf("sum_vec %f %f %f\n", fixed2double(sum_vec.n_vec.x), fixed2double(sum_vec.n_vec.y), fixed2double(sum_vec.n_vec.z));
	printf("start %f %f %f\n", fixed2double(screen.zero.x), fixed2double(screen.zero.y), fixed2double(screen.zero.z));
	printf("siye right %f siye down %f\n", fixed2double(screen.right.size), fixed2double(screen.down.size));
	return(screen);
}
...
t_ray ft_screen_ray(t_screen screen, int x, int y)
{
	...
	// printf("px point %f %f %f\n", fixed2double(px_point.x), fixed2double(px_point.y), fixed2double(px_point.z));
	// printf("camera %f %f %f\n", fixed2double(screen.camera.x), fixed2double(screen.camera.y), fixed2double(screen.camera.z));
	return(ft_ray(screen.camera, px_point));
}
